...
export const AdminSidebar: React.FC<SidebarProps> = ({ isCollapsed, onToggle }) => {
  ...
  const isActiveRoute = (to: string) => {
    // AJUSTARLO CUANDO ESTEMOS EN LA PANTALLA DE PRODUCTO 
        ...
        <div className="p-4 border-t border-gray-200">
          <div className="flex items-center space-x-3 p-3 rounded-lg hover:bg-gray-50 transition-colors cursor-pointer">
            <div className="w-10 h-10 bg-linear-to-br from-blue-500 to-purple-600 rounded-full flex items-center justify-center text-white font-semibold">
              {user?.fullName.substring(0, 2).toLocaleUpperCase() || "JD"}
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-gray-900 truncate">{user?.fullName || "Jhon Doe"}</p>
              <p className="text-xs text-gray-500 truncate"> {user?.email || "john@company.com"}</p>
            </div>
          </div>
        </div>
      )}
